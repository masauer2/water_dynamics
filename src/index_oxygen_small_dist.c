...
void calculate_OH3_distance(struct Frame *analysis_frame, double oxyCoords[], double hCoords[3][3]){
	...
	hCoords[0][0] = analysis_frame->x[hydrogen_idx[0]];
	hCoords[1][0] =analysis_frame->y[hydrogen_idx[0]];
	hCoords[2][0] = analysis_frame->z[hydrogen_idx[0]];
	hCoords[0][1] = analysis_frame->x[hydrogen_idx[1]];
	hCoords[1][1] =analysis_frame->y[hydrogen_idx[1]];
	hCoords[2][1] = analysis_frame->z[hydrogen_idx[1]];
	hCoords[0][2] = analysis_frame->x[hydrogen_idx[2]];
	hCoords[1][2] =analysis_frame->y[hydrogen_idx[2]];
	hCoords[2][2] = analysis_frame->z[hydrogen_idx[2]];
}
...
void arrange_hydrogens(double h_coords_new[3][3], double h_coords_old[3][3], double read_out[3][3]){
	for(int index = 0; index < 3; index++){
			double vec1[] = {h_coords_new[0][index], h_coords_new[1][index], h_coords_new[2][index]};
			double small = BOX_SIZE;
			int id = 0;
			double checker;
			for(int j = 0; j < 3; j++){
				double vec2[] = {h_coords_old[0][j], h_coords_old[1][j], h_coords_old[2][j]};
				checker = compute_distance(vec1, vec2);
				if (checker < small){
					small = checker;
					id = j;
				}
			}
			read_out[0][index] = h_coords_new[0][id];
			read_out[1][index] = h_coords_new[1][id];
			read_out[2][index] = h_coords_new[2][id];
		}
	}

void arrange_h2o(double h_coords_new[3][2],double h_coords_old[3][2], double read_out[3][2]){

	for(int j = 0; j < 2; j++){
			printf("COORDS (OLD): [%f %f %f]\n", h_coords_old[0][j], h_coords_old[1][j], h_coords_old[2][j]);
			printf("COORDS (NEW): [%f %f %f]\n", h_coords_new[0][j], h_coords_new[1][j], h_coords_new[2][j]);
		}
	...
	for(int index = 0; index < 2; index++){
			...
			for(int j = 0; j < 2; j++){
				double vec2[] = {h_coords_old[0][j], h_coords_old[1][j], h_coords_old[2][j]};
				checker = compute_distance(vec1, vec2);
				printf("Distance between index = %d and j = %d is %f\n", index, j, checker);
				if (checker < small){
					small = checker;
					id = j;
				}
...
int main(int argc, char* argv[]){
	...
	*/
	...
	struct Frame *last_frame =  malloc(sizeof(struct Frame));  // Frame to store coordinates	
	...
	for(int k = begin_frame; k < end_frame+1; k++){
		if(k != begin_frame){
			...
			current_frame =  malloc(sizeof(struct Frame));
		}
		...
		if(j == 0){
			...
			for (int index = 0; index < 4; index++){
				if(index == 0){
					...
					double oxygenCoordinates_new[] = {oxy[new][0][index], oxy[new][1][index], oxy[new][2][index]};

					
					calculate_OH3_distance(current_frame, oxygenCoordinates_new, h3c_new);
					
					for(int i = 0; i <3;i++){
						for(int j = 0; j < 3;j++){
							h3c_old[i][j] = h3c_new[i][j];
						}
					}
						
					fprintf(traj, "H\t%f\t%f\t%f\n", h3c_old[0][0], h3c_old[1][0], h3c_old[2][0]);
					fprintf(traj, "H\t%f\t%f\t%f\n", h3c_old[0][1], h3c_old[1][1], h3c_old[2][1]);
					fprintf(traj, "H\t%f\t%f\t%f\n", h3c_old[0][2], h3c_old[1][2], h3c_old[2][2]);
				...
				if(index != 0){
					...
					calculate_OH2_distance(current_frame, oxygenCoordinates, h2c_new);
					for(int i = 0; i <3;i++){
						for(int j = 0; j < 2;j++){
							h2c_old[i][j] = h2c_new[i][j];
						}
					}
					fprintf(traj, "H\t%f\t%f\t%f\n", h2c_old[0][0], h2c_old[1][0], h2c_old[2][0]);
					fprintf(traj, "H\t%f\t%f\t%f\n", h2c_old[0][1], h2c_old[1][1], h2c_old[2][1]);
				}
		...
		if(j != 0){
			double oxy_trajs[3][4];
			tracker = identify_traj(current_stack, oxy, oxy_trajs);
			...
			for (int index = 0; index < 4; index++){
				if(index == tracker){
					...
					oxy[new][0][index] = oxy_trajs[0][index];
					oxy[new][1][index] = oxy_trajs[1][index];
					oxy[new][2][index] = oxy_trajs[2][index];

					fprintf(traj, "O\t%f\t%f\t%f\n", oxy[new][0][index], oxy[new][1][index], oxy[new][2][index]);
			
					double oxygenCoordinates_old[] = {oxy[old][0][index], oxy[old][1][index], oxy[old][2][index]};
					double oxygenCoordinates_new[] = {oxy[new][0][index], oxy[new][1][index], oxy[new][2][index]};

					calculate_OH3_distance(last_frame, oxygenCoordinates_old, h3c_old);
					calculate_OH3_distance(current_frame, oxygenCoordinates_new, h3c_new);

					arrange_hydrogens(h3c_new, h3c_old, read_out);
						
					fprintf(traj, "H\t%f\t%f\t%f\n", read_out[0][0], read_out[1][0], read_out[2][0]);
					fprintf(traj, "H\t%f\t%f\t%f\n", read_out[0][1], read_out[1][1], read_out[2][1]);
					fprintf(traj, "H\t%f\t%f\t%f\n", read_out[0][2], read_out[1][2], read_out[2][2]);
					

					oxy[old][0][index] = oxy[new][0][index];
					oxy[old][1][index] = oxy[new][1][index];
					oxy[old][2][index] = oxy[new][2][index];
					h3c_old[0][index] = h3c_new[0][index];
					h3c_old[1][index] = h3c_new[1][index];
					h3c_old[2][index] = h3c_new[2][index];
				}
			}

			for (int index = 0; index < 4; index++){
				if(index != tracker){
					printf("Looking at Water #%d\n", index);
					oxy[new][0][index] = oxy_trajs[0][index];
					oxy[new][1][index] = oxy_trajs[1][index];
					oxy[new][2][index] = oxy_trajs[2][index];
					...
					double oxygenCoordinates_old[] = {oxy[old][0][index], oxy[old][1][index], oxy[old][2][index]};
					double oxygenCoordinates_new[] = {oxy[new][0][index], oxy[new][1][index], oxy[new][2][index]};
					//printf("New O coords: [%f %f %f]\n",oxy[new][0][index], oxy[new][1][index], oxy[new][2][index] );
					//printf("Old O coords: [%f %f %f]\n",oxy[old][0][index], oxy[old][1][index], oxy[old][2][index] );
					calculate_OH2_distance(last_frame, oxygenCoordinates_old, h2c_old);
					calculate_OH2_distance(current_frame, oxygenCoordinates_new, h2c_new);

					//printf("New H coords:\nH1:[%f %f %f]\nH2:[%f %f %f]\n", h2c_new[0][0],h2c_new[1][0] , h2c_new[2][0], h2c_new[0][1],h2c_new[1][1] , h2c_new[2][1]);
					//printf("Old H coords:\nH1:[%f %f %f]\nH2:[%f %f %f]\n", h2c_old[0][0],h2c_old[1][0] , h2c_old[2][0], h2c_old[0][1],h2c_old[1][1] , h2c_old[2][1]);
					arrange_h2o(h2c_new, h2c_old, read_out_2);

					fprintf(traj, "H\t%f\t%f\t%f\n", read_out_2[0][0], read_out_2[1][0], read_out_2[2][0]);
					fprintf(traj, "H\t%f\t%f\t%f\n", read_out_2[0][1], read_out_2[1][1], read_out_2[2][1]);

					oxy[old][0][index] = oxy[new][0][index];
					oxy[old][1][index] = oxy[new][1][index];
					oxy[old][2][index] = oxy[new][2][index];

					h2c_old[0][index] = h2c_new[0][index];
					h2c_old[1][index] = h2c_new[1][index];
					h2c_old[2][index] = h2c_new[2][index];
					}
