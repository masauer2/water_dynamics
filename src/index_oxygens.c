...
int identify_traj(struct Stack *current_stack, double os[][3][4], double traj[3][4]){
	...
	for(int i = 0; i < 128; i++){
		...
		else if(i != 0 && (i != o1_index && i != o2_index)){
	...
	for(int i = 0; i < 128; i++){
		...
		else if(i != 0 && (i != o1_index && i != o2_index && i != o3_index)){
...
void calculate_OH3_distance(struct Frame *analysis_frame, double oxyCoords[], double hCoords[][3]){
...
void calculate_OH2_distance(struct Frame *analysis_frame, double oxyCoords[], double hCoords[][2]){
	...
	for(int k = begin_frame; k < end_frame+1; k++){
		...
		if(j != 0){
			...
			for (int index = 0; index < 4; index++){
				...
				if(index == tracker){
					fprintf(traj, "H\t%f\t%f\t%f\n", h3c[0][0], h3c[1][0], h3c[2][0]);
					fprintf(traj, "H\t%f\t%f\t%f\n", h3c[0][1], h3c[1][1], h3c[2][1]);
					fprintf(traj, "H\t%f\t%f\t%f\n", h3c[0][2], h3c[1][2], h3c[2][2]);
				}
				if(index != tracker){
					fprintf(traj, "H\t%f\t%f\t%f\n", h2c[0][0], h2c[1][0], h2c[2][0]);
					fprintf(traj, "H\t%f\t%f\t%f\n", h2c[0][1], h2c[1][1], h2c[2][1]);
				}
